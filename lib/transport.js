...
Transport.prototype.onMessage = function (packet) {
  ...
  if ('heartbeat' == packet.type) {
    ...
    if ('disconnect' == packet.type && packet.endpoint == '') {
      ...
      if (current) {
        this.log.debug("transport handling force disconnect locally");
        current.onForcedDisconnect();
      } else {
        this.log.debug("transport publishing force disconnect");
    ...
    if (packet.id && packet.ack != 'data') {
      ...
      if (current && current.open) {
        this.log.debug("transport handling acking packet locally");
        current.onDispatch(ack);
      } else {
        this.manager.onClientDispatch(this.id, ack);
        this.log.debug("transport publishing dispatch:");
    ...
    if (current) {
      this.log.debug("transport handling client message locally");
      this.manager.onClientMessage(this.id, packet);
    } else {
      this.log.debug("transport publishing message:");
