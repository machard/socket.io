...
Transport.prototype.onMessage = function (packet) {
  ...
  if ('heartbeat' == packet.type) {
    ...
    if ('disconnect' == packet.type && packet.endpoint == '') {
      ...
      if (current) {
          console.log ("transport handling force disconnect locally");
    ...
    if (packet.id && packet.ack != 'data') {
      ...
      if (current && current.open) {
          console.log ("transport handling acking packet locally");
        current.onDispatch(ack);
      } else {
        this.manager.onClientDispatch(this.id, ack);
        console.log ("transport publishing dispatch:");
    ...
    if (current) {
      console.log ("transport handling client message locally");
      this.manager.onClientMessage(this.id, packet);
    } else {
      console.log ("transport publishing message:");
